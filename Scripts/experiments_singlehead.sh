    ...
    do
        ...
        if [ "${main_dataset}" != "${aux_dataset}" ]; then
            python ../main.py \
                ...
                --lr_finetune=${lr} \
                --workspace=../Workspaces/main${main_dataset}_aux${aux_dataset}_twostep_slanted_bruteforce_seed${SEED} \
                --lambdas=1.0,0.1 \
                ...
                --context_only \
                --two_step \
                --seed=${SEED} &> main${main_dataset}_aux${aux_dataset}_twostep_slanted_bruteforce_seed${SEED}_log.txt
        else
            python ../main.py \
                --dataset=${main_dataset} \
                --data_dir=../../../Data/ \
                --workspace=../Workspaces/main${main_dataset}_twostep_slanted_bruteforce_seed${SEED} \
                --lr_finetune=${lr} \
                --num_epochs_finetune=15 \
                --scheduler=slanted \
                --fuse_type=bruteforce \
                --context_only \
                --two_step \
                --seed=${SEED} &> main${main_dataset}_twostep_slanted_bruteforce_seed${SEED}_log.txt
        fi
    done
done
