...
class Trainer(object):
    ...
    def compute_metrics(self, labels, logits):
        ...
        confusion_matrix = np.zeros((n_rows, n_cols), dtype=np.int64)
        for i in range(n):
            row_idx = labels[i]
            col_idx = preds[i]
            confusion_matrix[row_idx, col_idx] += 1

        # rownorm_confusion_matrix = confusion_matrix / confusion_matrix.sum(axis=1, keepdims=True)
        # colnorm_confusion_matrix = confusion_matrix / confusion_matrix.sum(axis=0, keepdims=True)
        # print(confusion_matrix, rownorm_confusion_matrix, colnorm_confusion_matrix)
